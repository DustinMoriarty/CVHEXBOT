...
class serHandler(threading.Thread):
    ...
    def connect(self):
            ...
            for port in comList:
                    try:
                            ser = serial.Serial(port, baudrate= BAUD_RATE, timeout=2, writeTimeout = 3) #Altered By Dustin Moriarty
                            ser.write('V\n')
                            result = ser.readline()
            ...
            if self.serOpen == False:
                ...
                for i in range(1,100):
                    try:
                        try:
                            ser = serial.Serial(i, baudrate=BAUD_RATE, timeout=1, writeTimeout = 10) #Altered By Dustin Moriarty
                            print "ser",i
                        except:
                            print "ser",i,"failed"
                            raise Exception
...
class Controller:
    def __init__(self,servos=32):
        ...
        timeout = time.time()
        #while not (self.serialHandler.serOpen or (time.time()-timeout > 10.0)): #Commented Out By Dustin Moriarty
        while not (self.serialHandler.serOpen or ((time.time()-timeout) > 10.0)): #Added By Dustin Moriarty
            time.sleep(1) #Changed To 1 Second By Dustin Moriarty To Conserve Resources
